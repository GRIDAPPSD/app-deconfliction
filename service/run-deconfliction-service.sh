...
# ./run-deconfliction-service.sh <MODEL> <APPS> [--optlib <OPTLIB>]
#                                               [--interval <INTERVAL>]
#                                               [--weights <WEIGHTS>]
#
...
#
# e.g.,
# ./run-deconfliction-service.sh 123apps rm

if [ "$#" -lt 2 ]; then
  echo "Usage: ./run-deconfliction-service.sh <model> <apps_code> [--optlib <opt_library>] [--interval <interval_sec>] [--weights <weights_basename>]"
  echo
  exit
fi
...
# Because it's a service, the pipeline is run within the docker container so
# don't start it here
