...
class SwitchAreaAgentLevelRbdmService(SwitchAreaAgent):
    ...
    def __init__(self,
        ...
        if self.switch_area.container.mRID == '_E3D03A27-B988-4D79-BFAB-F9D37FB289F7.0':
            self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'distributed_123' / 'master_2.dss'
        elif self.switch_area.container.mRID == '_E3D03A27-B988-4D79-BFAB-F9D37FB289F7.1':
            self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'distributed_123' / 'master_5.dss'
        elif self.switch_area.container.mRID == '_E3D03A27-B988-4D79-BFAB-F9D37FB289F7.2':
            self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'distributed_123' / 'master_3.dss'
        elif self.switch_area.container.mRID == '_E3D03A27-B988-4D79-BFAB-F9D37FB289F7.6':
            self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'distributed_123' / 'master_4.dss'
        elif self.switch_area.container.mRID == '_E3D03A27-B988-4D79-BFAB-F9D37FB289F7.7':
            self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'distributed_123' / 'master_1.dss'
        else:
            self.openDssFile = None

        print(self.openDssFile)
        if self.openDssFile is not None:
            dss.Basic.AllowChangeDir(False)
            self.dssContext = dss.NewContext()
            self.dssContext.run_command("Clear")
            self.dssContext.run_command(f'Redirect {str(self.openDssFile)}')
            self.dssContext.run_command(f'Compile {str(self.openDssFile)}')
            self.dssContext.Solution.Mode(0)
            # self.dssContext.Solution.SolveNoControl()
            self.dssContext.Solution.Solve()
    ...
    def on_upstream_message(self, headers: Dict, message: Dict):
        if message.get("requestType", "") == "Deconflict":
            ...
            print(f'returned resolution vector message for {self.switch_area.container.mRID}')
    ...
    def runPowerFlowSnapshot(self, setpoints: List, setpointNames: List, simulationData: Dict):
        ...
        print(f'running snapshot for f{self.switch_area.container.mRID}')
        # self.dssContext.Solution.SolveNoControl()
        self.dssContext.Solution.Solve()
    ...
    def buildUMatrix(self, setpoints: List, setpointNames: List, index: int, simulationData: Dict):
        ...
        if not violations:
            ...
            self.uMatrix[index, 3] = profit
            self.uMatrix[index, 0] = -p_loss
            self.uMatrix[index, 2] = -emissions
            self.uMatrix[index, 1] = total_batt
        else:
    ...
    def normalizeUMatrix(self):
        ...
        if self.uMatrix.size == 0:  # Throw error if all setpoint alternaitives caused powerflow operation violations
            print("!\n!\n!WARNING:  All the setpoints requested by the applications are causing powerflow operation "
                  f"violations! Sending Resolution from previous time in {self.switch_area.container.mRID}.\n!\n!")
            return
    ...
    def deconflict(self, conflictMatrix: Dict):
        print(f'entered deconflictor for SA {self.switch_area.container.mRID}')
        ...
        # self.dssContext = dss.NewContext()
        # self.dssContext.run_command("Clear")
        # self.openDssFile = Path(__file__).parent.resolve() / '123Bus' / 'Run_IEEE123Bus.dss'
        # self.dssContext.run_command(f'Redirect {str(self.openDssFile)}')
        # self.dssContext.run_command(f'Compile {str(self.openDssFile)}')
        # self.dssContext.Solution.SolveNoControl()
        if self.openDssFile is not None:
            ...
            if len(distributedConflictMatrix.get("setpoints", {}).keys()) > 0:
                ...
                #-----------------------------------------------------------------------
                print(f'dispatching solar for f{self.switch_area.container.mRID}')
                ...
                self.uMatrix = np.empty((self.numberOfSets, 4))
                self.invalidSetpoints = []
                ...
                if self.uMatrix.size > 0:
                    ...
                    for i in range(len(self.setpointSetVector["setpointIndexMap"])):
                        resolutionVector["setpoints"][self.setpointSetVector["setpointIndexMap"][i]] = 0 #\
            ...
            print(f'extracting losses for f{self.switch_area.container.mRID}')
            p_loss = self.extractPowerLosses()
            print(f'extracting profit for f{self.switch_area.container.mRID}')
            profit, emissions = self.extractProfitAndEmissions()
            print(f'extracting battery for f{self.switch_area.container.mRID}')
            total_batt = self.extractBatteryOutput()
