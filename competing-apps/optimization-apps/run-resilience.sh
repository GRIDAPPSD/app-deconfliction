...
if [ "$1" = "pulp" ] || [ "$1" = "PULP" ] || [ "$1" = "PuLP" ]; then
  python3 optimization-app-pulp.py resilience $SIMID "$SIMREQ" $STATE 2>&1 | tee output/resilience-app.log
else
  python3 optimization-app-cvxpy.py resilience $SIMID "$SIMREQ" $STATE 2>&1 | tee output/resilience-app.log
fi
#python3 optimization-app-pulp.py resilience $SIMID "$SIMREQ" $STATE --outage 56 68 2>&1 | tee output/resilience-app.log
